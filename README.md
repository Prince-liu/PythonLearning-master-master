...
- **智能测点布局**：
  - 网格布点（均匀/变间距，可设置边距）
  - 极坐标布点（圆心自动判断，支持多层多点，可变半径步长）
  - 自定义布点（CSV导入）
  - 路径优化（之字形/最近邻/螺旋）
- **自动数据采集**：多点顺序采集，实时进度显示，质量检查
- **信号处理配置**：小波降噪、带通滤波，配置随实验保存
- **空间插值**：IDW、Kriging、RBF、Linear、Cubic 多种插值算法，可选高斯平滑
- **应力云图**：彩色等高线可视化，可调色阶，支持导出图片
- **实验管理**：创建、加载、删除、导出实验数据，所有配置参数持久化保存
