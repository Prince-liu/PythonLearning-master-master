...
> **包含章节**：第7章 实时云图算法、第8章 数据结构  
> **上一部分**：[第2部分](./单轴应力检测模块设计文档-第2部分.md)  
> **下一部分**：[第4部分](./单轴应力检测模块设计文档-第4部分.md)
...
**设计原则**：
- **只显示有实测数据的区域**（测试区域）
- **边距区域不显示云图**（无数据区域，避免外推误导）
- **清晰区分**：用不同颜色标识有数据区和无数据区
...
```
样件 100×150mm，边距 10mm，测试区域 80×130mm
...
**科学性说明**：
- ✅ **不做无根据的外推**：边距区域没有测点，不显示插值结果
- ✅ **清晰标识**：用户一眼看出哪些区域有数据，哪些没有
- ✅ **避免误导**：防止用户误以为边距区域的应力值是真实测量的
...
**📄 文档说明**：本文档为第3部分，包含第7-8章内容。  
**⬅️ 上一部分**：[单轴应力检测模块设计文档-第2部分.md](./单轴应力检测模块设计文档-第2部分.md)  
**➡️ 下一部分**：[单轴应力检测模块设计文档-第4部分.md](./单轴应力检测模块设计文档-第4部分.md)
