...
> **模块名称**：stress_detection_uniaxial  
> **文档版本**：v1.0  
> **创建日期**：2025-12-19  
> **最后更新**：2025-12-19
...
1. [模块概述](#1-模块概述)
2. [代码架构设计](#2-代码架构设计)
3. [界面设计](#3-界面设计) - 见第1.5部分
4. [形状定义功能](#4-形状定义功能) - 见第2部分
5. [布点策略](#5-布点策略) - 见第2部分
6. [采集流程](#6-采集流程) - 见第2部分
7. [实时云图算法](#7-实时云图算法) - 见第3部分
8. [数据结构](#8-数据结构) - 见第3部分
9. [API接口设计](#9-api接口设计) - 见第3部分
10. [实现计划](#10-实现计划) - 见第3部分
...
### 1.1 功能定位
单轴应力检测模块用于基于已标定系数的应力测量，包含**单点实时检测**和**多点应力场测绘**两大功能。
...
### 1.3 应用场景
- **单点检测**：实时监控单点应力变化、质量控制、快速检测
- **应力场测绘**：平板应力分布、孔洞应力集中、复杂形状应力场研究

### 1.4 技术依赖
- **前置模块**：应力标定模块（必须先完成标定）、实时采集模块（单点检测需要）
- **核心算法**：互相关时间差计算、scipy.interpolate.griddata（插值）
- **数据存储**：SQLite（元数据）+ HDF5（波形数据）
- **新增依赖**：shapely（几何运算）
...
├── __init__.py                     # 模块导出
├── field_experiment.py             # 实验管理（200行）
├── field_capture.py                # 采集控制（250行）
├── shape_utils.py                  # 形状工具（200行）
├── point_generator.py              # 测点生成（200行）
├── interpolation.py                # 插值算法（150行）
└── contour_generator.py            # 云图生成（150行）

说明:
- 采用模块化设计，每个文件职责单一
- 每个文件 150-250 行，易于维护和测试
- 工具文件提供纯函数，业务文件管理状态
...
class FieldExperiment:
    ...
    def __init__(self, data_manager):
        """初始化
        参数: data_manager - 数据管理器实例（与标定模块共用）
        """
    ...
    def create_experiment(self, name, description, calibration_data, shape_config) -> dict
        ...
        参数:
            name - 实验名称
            description - 实验描述
            calibration_data - 标定数据（应力系数k）
            shape_config - 试件形状配置
        返回: {"success": bool, "exp_id": str, "message": str}
        """
    ...
    def load_calibration_from_experiment(self, calib_exp_id, direction) -> dict
        ...
        参数:
            calib_exp_id - 标定实验ID（如 3 表示 EXP003）
            direction - 测试方向（如 "0°"）
        
        返回: {"success": bool, "data": {...}}
        
        实现原理:
        1. 通过 data_manager 访问 experiments.db
        2. 查询 test_directions 表获取方向ID
        3. 查询 fitting_results 表获取拟合结果
        4. 计算应力系数 k = 1/斜率
        ...
        返回数据:
            {
                "k": float,              # 应力系数 (MPa/ns)
                "slope": float,          # 原始斜率 (ns/MPa)
                "intercept": float,      # 截距 (ns)
    ...
    def load_calibration_from_file(self, file_path) -> dict
        ...
        返回: {"success": bool, "data": {"k": float, "source": "file", ...}}
        """
    ...
    def validate_calibration_data(self, calibration_data) -> dict
        ...
        检查项:
        - 应力系数 k 是否合理（通常 0.1-10 MPa/ns）
        - 拟合优度 R² 是否足够高（建议 > 0.95）
        - 数据来源是否可靠
    ...
    def export_experiment(self, exp_id, format, options) -> dict
        ...
        返回: {"success": bool, "file_path": str}
        """
    
    def get_experiment_list(self, status_filter=None) -> list
        """获取实验列表
        参数: status_filter - 'in_progress' | 'completed' | None
        返回: [{"exp_id": str, "name": str, "status": str, ...}, ...]
        """
    
    def update_experiment_status(self, exp_id, status) -> dict
        """更新实验状态
        参数: status - 'in_progress' | 'completed'
        """
    
    def get_experiment_statistics(self, exp_id) -> dict
        """获取实验统计信息
        返回: {"total_points": int, "completed": int, "skipped": int,
...
class FieldCapture:
    ...
    def __init__(self, oscilloscope, data_manager, signal_processor):
        ...
        self.monitoring = False  # 实时监控状态
    
    def start_monitoring(self) -> dict
        """开始实时监控
        前端订阅实时采集模块的波形更新，显示NORM模式波形
    ...
    def set_denoise_config(self, config) -> dict
        """设置降噪参数
        参数: config - {"method": str, "wavelet": str, "level": int, 
    ...
    def test_denoise_effect(self) -> dict
        ...
        2. 应用当前降噪参数
        3. 计算信噪比提升
        返回: {"success": bool, "original_snr": float, "denoised_snr": float,
    ...
    def _get_default_denoise_config(self) -> dict
        """获取默认降噪配置
        返回: {"method": "wavelet", "wavelet": "sym6", "level": 5,
...
def validate_shape(shape_config) -> dict
    """验证形状配置的有效性
    参数: shape_config - {"type": str, "params": dict, "modifiers": list}
    返回: {"is_valid": bool, "area": float, "warnings": list}
...
def create_shape_mask(shape_config, resolution) -> np.ndarray
    ...
    参数: 
        shape_config - 形状配置
        resolution - 网格分辨率 (nx, ny)
    返回: 布尔数组 (ny, nx)，True表示有效区域
    
    用途: 插值时只在有效区域内计算，孔洞区域不插值
    """
...
def apply_boolean_operations(base_shape, modifiers) -> shapely.Polygon
    """应用布尔运算（孔洞、缺口）
    参数:
        base_shape - 基础形状（矩形/圆形/多边形）
        modifiers - 修改器列表 [{"type": "hole", "shape": {...}}, ...]
    返回: shapely.Polygon 对象
    
    使用 shapely 库进行几何运算:
    - 孔洞: base_shape.difference(hole_shape)
    - 缺口: base_shape.difference(notch_shape)
    """
```
...
def generate_grid_points(shape_config, rows, cols, row_spacing, col_spacing, margins) -> list
    ...
    参数:
        shape_config - 形状配置
        rows, cols - 行列数
        row_spacing - 行间距（可以是数组，支持变间距）
        col_spacing - 列间距（可以是数组，支持变间距）
        margins - 边距 {"top": float, "bottom": float, "left": float, "right": float}
    返回: [{"id": int, "x": float, "y": float}, ...]
    
    算法:
    1. 计算测点区域（扣除边距）
    2. 根据行列数和间距生成网格
    3. 过滤掉形状外的点（使用 is_point_inside）
    """
...
def generate_polar_points(center, r_start, r_end, n_rings, angle_range, points_per_ring) -> list
    ...
    参数:
        center - 圆心坐标 (x, y)
        r_start, r_end - 起始/结束半径
        n_rings - 径向层数
        angle_range - 角度范围 (start_deg, end_deg)
        points_per_ring - 每层点数（可以是数组，支持每层不同）
    返回: [{"id": int, "x": float, "y": float, "r": float, "theta": float}, ...]
...
def generate_adaptive_points(shape_config, dense_region, n_dense, n_sparse) -> list
    ...
    参数:
        shape_config - 形状配置
        dense_region - 密集区域 {"center": (x, y), "radius": float}
        n_dense - 密集区测点数
        n_sparse - 外围测点数
    返回: [{"id": int, "x": float, "y": float}, ...]
    
    算法:
    1. 在密集区域内生成 n_dense 个随机点（或小网格）
    2. 在外围区域生成 n_sparse 个均匀点
    3. 合并并优化顺序
    """
...
def interpolate_stress_field(points, shape_config, method='cubic', resolution=100) -> dict
    """插值生成应力场
    参数:
        points - 已测点数据 [{"x": float, "y": float, "stress": float}, ...]
        shape_config - 形状配置（用于生成遮罩）
        method - 'linear' | 'cubic' | 'rbf'（径向基函数）
        resolution - 网格密度（每边点数）
    返回: {"xi": array, "yi": array, "zi": array, "mask": array}
    
    算法:
    1. 使用 scipy.interpolate.griddata 进行插值
    2. 应用形状遮罩，孔洞区域设为 NaN
    3. 返回插值网格数据
    """
...
def get_confidence_level(n_points, shape_area) -> str
    """根据测点数和面积返回置信度等级
    规则:
        - < 3 点: 'insufficient'（不足，无法插值）
        - 3-8 点: 'preview'（预览，仅供参考）
        - >= 9 点: 'full'（完整，可信度高）
        - 密度 > 1点/100mm²: 'high_density'（高密度）
    """
...
def generate_contour_lines(xi, yi, zi, levels=10) -> list
    ...
    参数:
        xi, yi, zi - 插值网格数据
        levels - 等高线层数
    返回: [{"level": float, "paths": [[(x, y), ...], ...]}, ...]
    
    使用 matplotlib.pyplot.contour 生成等高线
    """
```
...
class ContourGenerator:
    ...
    def generate_contour(self, points, shape_config, method='cubic', resolution=50, 
        ...
        """生成云图数据
        流程:
        1. 调用 interpolate_stress_field 插值
        2. 应用形状遮罩
        3. 生成等高线
        4. 生成色标映射
        5. 返回可视化数据
    ...
    def get_color_map(self, stress_values, vmin, vmax, colormap='jet') -> dict
        ...
        参数:
            stress_values - 应力值数组
            vmin, vmax - 色标范围（None表示自动）
            colormap - 色图名称
        返回: {"colors": array, "levels": array, "colormap": str}
        """
    
    def export_contour_image(self, contour_data, shape_config, file_path, dpi=300) -> dict
        """导出云图为图片
        使用 matplotlib 绘制并保存为 PNG
        包含: 试件轮廓、测点位置、应力云图、色标、标题
        """
...
├── stress-detection-uniaxial.js    # 主模块（200行）
├── field-experiment-manager.js     # 实验管理UI（200行）
├── field-calibration-panel.js      # 标定数据面板（150行）
├── field-shape-panel.js            # 形状定义面板（200行）
├── field-layout-panel.js           # 布点设置面板（200行）
├── field-capture-panel.js          # 采集控制面板（250行）
├── field-canvas.js                 # 预览画布（200行）
└── field-contour.js                # 云图显示（200行）

说明:
- 采用模块化设计，每个文件对应一个UI面板或功能
- 每个文件 150-250 行，职责单一
- 使用 IIFE 模式封装，避免全局命名空间污染
...
const FieldShapePanel = (function() {
    ...
    async function validateShape() {
        ...
        if (response.success) {
            showToast(`✓ 形状有效: 面积 ${response.area} mm²`, 'success');
            FieldCanvas.drawShape(shapeConfig);
        } else {
            showToast(`❌ 形状无效: ${response.message}`, 'error');
        }
    }
...
const FieldLayoutPanel = (function() {
    ...
    async function generatePoints() {
        ...
        if (response.success) {
            ...
            showToast(`✓ 已生成 ${points.length} 个测点`, 'success');
...
const FieldCapturePanel = (function() {
    /**
     * 采集控制：实时监控、测点采集、降噪设置、进度管理
     */
    
    let captureState = 'waiting';  // 'waiting' | 'capturing' | 'confirming'
    let monitoringState = false;   // 实时监控状态
    ...
    function startMonitoring() {
        ...
        // 订阅实时采集模块的波形更新（NORM模式，快速刷新）
        RealtimeCapture.订阅波形更新(handleWaveformUpdate);
    }
    ...
    function stopMonitoring() {
        ...
        // 取消订阅
        RealtimeCapture.取消订阅波形更新(handleWaveformUpdate);
    }
    ...
    function handleWaveformUpdate(data) {
        ...
        const { 波形数据, 显示状态 } = data;
        
        // 在右上角"当前波形"区域显示实时波形
        FieldCanvas.drawWaveform(波形数据, 显示状态);
    }
    ...
    async function captureCurrentPoint() {
        ...
        // 获取RAW模式高精度波形（12bit，完整存储深度）
        const rawResult = await RealtimeCapture.获取当前RAW波形();
        
        if (!rawResult.success) {
            captureState = 'waiting';
            if (wasMonitoring) monitoringState = true;
            showToast(`采集失败: ${rawResult.message}`, 'error');
            updateUI();
            return;
        }
        
        // 调用后端采集接口（包含降噪、互相关、应力计算）
        ...
        if (response.success) {
            ...
            captureState = 'waiting';
            if (wasMonitoring) monitoringState = true;
            showToast(`采集失败: ${response.message}`, 'error');
        }
        updateUI();
    }
    ...
    async function confirmAndContinue() {
        ...
        if (currentPointIndex < points.length) {
            showToast('已保存，请移动到下一测点', 'success');
        } else {
            showToast('所有测点采集完成！', 'success');
        }
    }
...
const SinglePoint = (function() {
    ...
    function handleWaveformUpdate(data) {
        // 绘制波形
        drawWaveform(data.波形数据, data.显示状态);
        
        // 如果正在检测，计算应力值
        if (isDetecting) {
            calculateStress(data.波形数据);
        }
    }
...
const FieldMapping = (function() {
    ...
    function addModifier(modifier) {}  // 添加孔洞/缺口
    ...
    async function captureCurrentPoint() {
        // 从实时采集模块获取波形
        const waveform = await RealtimeCapture.获取当前RAW波形();
        // 计算时间差和应力值
        // 保存数据
        // 更新云图
    }
...
```
static/css/
└── stress-detection.css    # 统一样式（包含单点和多点）
```
...
#### 现有代码处理
```
现有文件: static/js/stress-detection-uniaxial/stress-detection-uniaxial.js (500行)

拆分为:
1. stress-detection-uniaxial.js (新，200行)
   - 标签页切换逻辑
   - 模块初始化

2. single-point.js (300行)
   - 完整保留现有的单点检测代码
   - 包括订阅波形、绘制、计算、记录等所有功能

3. field-mapping.js (新增，600行)
   - 全新的应力场测绘功能

4. contour-drawing.js (新增，200行)
   - 云图绘制工具函数
```

#### 关键接口保持不变
- `RealtimeCapture.订阅波形更新()` - 单点检测继续使用
- `RealtimeCapture.获取当前RAW波形()` - 应力场测绘采集时使用
- `CommonUtils.绘制波形到画布()` - 两个功能共用
...
#### 后端
- stress_detection_uniaxial.py: ~600行
- shape_utils.py: ~200行
- point_generator.py: ~200行
- interpolation.py: ~150行
- **总计：~1150行**

#### 前端
- stress-detection-uniaxial.js: ~200行
- single-point.js: ~300行
- field-mapping.js: ~600行
- contour-drawing.js: ~200行
- **总计：~1300行**

---

**📄 文档说明**：本文档为第1部分，包含第1-2章内容。  
**➡️ 下一部分**：[单轴应力检测模块设计文档-第1.5部分.md](./单轴应力检测模块设计文档-第1.5部分.md)（第3章 界面设计）
