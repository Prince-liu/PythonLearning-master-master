...
canvas.addEventListener('click', (e) => {
    ...
    // 转换为数据坐标
    const dataX = pixelToDataX(x);
    const dataY = pixelToDataY(y);
    
    // 查找最近的测点
    const nearestPoint = findNearestPoint(dataX, dataY, threshold=10);
    
    if (nearestPoint) {
        showPointDetails(nearestPoint);
    }
});
...
function showPointDetails(point) {
    ...
    popup.innerHTML = `
        <div class="popup-header">
            <span>测点 #${point.id}</span>
            <span class="close-btn">×</span>
        </div>
        <div class="popup-body">
            <table>
                <tr><td>坐标:</td><td>(${point.x.toFixed(1)}, ${point.y.toFixed(1)}) mm</td></tr>
                <tr><td>时间差:</td><td>${point.timeDiff.toFixed(2)} ns</td></tr>
                <tr><td>应力值:</td><td>${point.stress.toFixed(2)} MPa</td></tr>
                <tr><td>质量评分:</td><td>${(point.qualityScore * 100).toFixed(0)}%</td></tr>
                <tr><td>测量时间:</td><td>${point.measuredAt}</td></tr>
            </table>
            <div class="popup-actions">
                <button onclick="viewWaveform(${point.id})">查看波形</button>
                <button onclick="remeasurePoint(${point.id})">重新测量</button>
            </div>
        </div>
    `;
...
canvas.addEventListener('mousemove', (e) => {
    ...
    hoverTimeout = setTimeout(() => {
        ...
        // 获取该位置的插值应力值
        const stress = getInterpolatedStress(dataX, dataY);
        
        if (stress !== null) {
            showTooltip(e.clientX, e.clientY, `应力: ${stress.toFixed(2)} MPa`);
        }
    }, 100);  // 100ms延迟，避免频繁更新
});
...
function showTooltip(x, y, text) {
    let tooltip = document.querySelector('.contour-tooltip');
    if (!tooltip) {
        tooltip = document.createElement('div');
        tooltip.className = 'contour-tooltip';
...
function hideTooltip() {
    const tooltip = document.querySelector('.contour-tooltip');
    if (tooltip) {
...
class ContourZoom {
    ...
    redraw() {
        ...
        // 绘制云图
        drawContourMap(ctx, contourData);
...
// 初始化缩放功能
const contourZoom = new ContourZoom(document.querySelector('#contour-canvas'));
```

**4. 导出高分辨率图片**
```javascript
async function exportContourImage(format='png', dpi=300) {
    """导出高分辨率云图"""
    // 创建高分辨率离屏Canvas
    const scale = dpi / 96;  // 96 DPI是屏幕默认分辨率
    ...
    if (format === 'png') {
        ...
        downloadImage(dataURL, `contour_${experimentId}.png`);
    } else if (format === 'svg') {
        // 使用后端生成SVG
        const response = await pywebview.api.export_contour_svg(experimentId);
        if (response.success) {
...
.point-details-popup .popup-header {
    padding: 12px 16px;
    background: #f5f5f5;
...
.point-details-popup .close-btn {
    cursor: pointer;
    font-size: 20px;
    color: #757575;
}

.point-details-popup .popup-body {
    padding: 16px;
}

.point-details-popup table {
    width: 100%;
    border-collapse: collapse;
}

.point-details-popup table td {
    padding: 6px 0;
}

.point-details-popup table td:first-child {
    font-weight: bold;
    width: 80px;
}

.point-details-popup .popup-actions {
    margin-top: 12px;
    display: flex;
    gap: 8px;
}

.point-details-popup .popup-actions button {
    flex: 1;
    padding: 8px;
    border: none;
    border-radius: 4px;
...
.point-details-popup .popup-actions button:hover {
    background: #1976d2;
}
