...
const FieldCapturePanel = (function() {
    ...
    async function 采集当前测点(isRecapture = false) {
        ...
        // 检查标定数据是否已加载（兼容旧逻辑，允许负数标定系数）
        if (!实验状态.标定数据 || 实验状态.标定系数 === 0 || 实验状态.标定系数 === null || 实验状态.标定系数 === undefined) {
            callbacks?.显示状态信息('⚠️', '请先加载标定数据', '需要标定系数才能计算应力值', 'warning');
            return;
        }
    ...
    async function 开始采集流程() {
        // 🆕 验证：必须先加载标定数据（允许负数标定系数）
        const k = 实验状态.标定系数;
        if (!实验状态.工作流程.已加载标定 || !实验状态.标定数据 || k === 0 || k === null || k === undefined) {
